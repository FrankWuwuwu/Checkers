...
def CNN(node):
    ...
    for child in node.children():
        state_list.append(child.board)
    states=[]
    for item in state_list:
        states.append(item.reshape(1,10,10))
    states=tr.tensor(np.array(states))
    out=model(states.float())
...
def decide_action(node):
    ...
    all_action = valid_actions(state)
    return all_action[np.argmax(out.detach().numpy())]
...
def get_action(node):
    ...
    if action_type==1:
        type_text="jump"
    print ("tree+NN AI action:",type_text,"the checker of",start,"to",end)
    
    return make_move

def treeNN_AI(state,rolltime):
